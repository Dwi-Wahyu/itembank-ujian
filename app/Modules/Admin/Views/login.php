...
<head>
  ...
  <title>Login Admin - E-Ujian</title>
  <?= csrf_meta() ?>
...
.btn.brand-btn.disabled {
  ...
  color: #fff !important;
  opacity: 1 !important;          /* <-- kunci anti-pudar */
  cursor: wait;
}
...
<body>
          ...
          <div class="card border-0 shadow-soft rounded-4">
            <div class="card-body p-4 p-md-5">
              <div class="text-center mb-4">
                ...
                <p class="small-muted mb-0">Masuk ke panel administrasi.</p>
              </div>
              ...
              <form id="adminLoginForm" autocomplete="off">
               ...
               <button type="submit" class="btn brand-btn w-100 py-2 fw-semibold rounded-3" id="btnLoginAdmin">
 ...
 function setLoading(on){
  const $b = $('#btnLoginAdmin');
  if (on){
    $b.prop('disabled', true);
    $b.find('.btn-label').text('Memproses...');
    $b.find('.btn-spinner').removeClass('d-none');
  } else {
    $b.prop('disabled', false);
    $b.find('.btn-label').text('Login');
    $b.find('.btn-spinner').addClass('d-none');
  }
}
...
$('#adminLoginForm').on('submit', function(e){
  ...
  $.ajax({
    ...
    if(res.status==='ok'){
      ...
      return; // tidak perlu setLoading(false) karena akan pindah halaman
    }
    ...
    // hanya aktif lagi kalau tidak redirect
    setLoading(false);
  });
});
