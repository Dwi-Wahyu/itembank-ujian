...
<div class="d-flex align-items-center justify-content-between mb-3">
  <h2 class="page-title">Sesi Ujian Praktek</h2>
  <div>
...
<form id="filterForm" class="row g-2 align-items-end mb-3" method="get" action="<?= base_url('admin/ujian/praktek') ?>">
  ...
  <div class="col-md-3">
    ...
    <a class="btn btn-link mt-2" href="<?= base_url('admin/ujian/praktek?tab='.$tab) ?>">Reset</a>
  </div>
</form>

<!-- container yang akan di-reload AJAX -->
<div id="praktekList">
  <?= view('\Modules\Admin\Views\ujian\partials\praktek_table', get_defined_vars()) ?>
</div>

<!-- MODAL TAMBAH -->
<div class="modal fade" id="modalAdd" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <form class="modal-content" id="formAddpraktek" autocomplete="off">
      <?= csrf_field() ?>
      <div class="modal-header">
        <h5 class="modal-title">Ujian praktek</h5>
...
<div class="modal fade" id="modalEdit" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <form class="modal-content" id="formEditpraktek" autocomplete="off">
      <?= csrf_field() ?>
      <input type="hidden" name="id" id="edit_id">
      <div class="modal-header">
        <h5 class="modal-title">Ubah Ujian praktek</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
        <div class="mb-3">
...
<div class="modal fade" id="importOfflineModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <form class="modal-content" id="formImportOffline" method="post" action="<?= base_url('admin/ujian/praktek/import-offline') ?>" enctype="multipart/form-data">
      <?= csrf_field() ?>
      <div class="modal-header">
...
(function(){
  ...
  $('#filterForm').on('submit', function(e){
    e.preventDefault();
    const base = '<?= base_url('admin/ujian/praktek') ?>';
...
$('#btnGenKode').on('click', function(){
  ...
  $(document).on('click', '.btn-edit', function(){
    ...
    Loader.show();
    $.get('<?= base_url('admin/ujian/praktek/get') ?>/' + id)
      .done(function(res){
        if(res.status === 'ok'){
          const d = res.data;
          $('#edit_id').val(d.id);
