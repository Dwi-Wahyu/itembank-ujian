...
<div class="d-flex align-items-center justify-content-between mb-3">
  <h2 class="page-title mb-0">Master Kompetensi Utama</h2>
  ...
  <div class="card-body">
    <form class="row g-2" id="frmSearch">
      ...
      <div class="col-md-6 text-md-end">
        ...
        <a href="<?= site_url('admin/master/kom-utama') ?>" class="btn btn-link">Reset</a>
      </div>
    </form>
  </div>
</div>
...
<div id="listWrap">
  <?= view('\Modules\Admin\Views\ref\partials\kom_utama_table', get_defined_vars()) ?>
</div>

<!-- MODAL Tambah/Edit -->
<div class="modal fade" id="modalKom" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog">
    <form class="modal-content" id="formKom" autocomplete="off">
      <?= csrf_field() ?>
      <input type="hidden" name="id" id="id">
      <div class="modal-header">
        <h5 class="modal-title">Kompetensi Utama</h5>
...
(function(){
  const $wrap   = $('#listWrap');
  const modalEl = document.getElementById('modalKom');
  const modal   = new bootstrap.Modal(modalEl);

  function buildURL(){
    const base = '<?= site_url('admin/master/kom-utama') ?>';
  ...
  $(document).on('click','.btn-edit', function(){
    ...
    $.get('<?= site_url('admin/master/kom-utama/get') ?>/'+id)
      .done(function(res){
        if(res.status==='ok'){
          $('#id').val(res.data.id);
  ...
  $('#formKom').on('submit', function(e){
    ...
    $.ajax({
      url:'<?= site_url('admin/master/kom-utama/save') ?>',
