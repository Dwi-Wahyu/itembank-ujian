<?php
// $rows, $total, $page, $per, $q
$pages = max(1, (int)ceil(($total ?: 0) / ($per ?: 20)));
function qurl_bid($p=[]){ return current_url().'?'.http_build_query(array_merge($_GET,$p)); }
?>
<div class="card">
        ...
        <tr>
          <td>
            <div class="btn-group btn-group-sm">
              ...
              <button class="btn btn-outline-danger btn-del" data-id="<?= $r['id'] ?>" title="Hapus">
  ...
  <div class="card-body d-flex justify-content-between align-items-center flex-wrap gap-2">
    ...
    <nav>
      <?= render_pagination($page, $pages, function($p) { return qurl_bid(['page' => $p]); }) ?>
    </nav>
  </div>
</div>
