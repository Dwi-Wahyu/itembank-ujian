...
<div class="d-flex align-items-center justify-content-between mb-3">
  <h2 class="page-title mb-0">Master Kelompok Penyakit</h2>
...
<div class="card mb-3">
  <div class="card-body">
    <form class="row g-2" id="frmSearch">
      <div class="col-md-6">
        <input type="text" class="form-control" name="q" value="<?= esc($q ?? '') ?>" placeholder="Cari kode/nama…">
      </div>
      <div class="col-md-6 text-md-end">
        <button class="btn btn-outline-primary"><i class="bi bi-search"></i> Cari</button>
        <a href="<?= site_url('admin/master/kel-penyakit') ?>" class="btn btn-link">Reset</a>
      </div>
    </form>
  </div>
</div>
...
<div id="listWrap">
  <?= view('\Modules\Admin\Views\ref\partials\kel_penyakit_table', get_defined_vars()) ?>
</div>

<!-- MODAL: Tambah/Edit -->
<div class="modal fade" id="modalKelP" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog">
    <form class="modal-content" id="formKelP" autocomplete="off">
      <?= csrf_field() ?>
      <input type="hidden" name="id" id="id">
      <div class="modal-header">
        <h5 class="modal-title">Kelompok Penyakit</h5>
...
(function(){
  const $wrap   = $('#listWrap');
  const modalEl = document.getElementById('modalKelP');
  const modal   = new bootstrap.Modal(modalEl);

  function buildURL(){
    const base = '<?= site_url('admin/master/kel-penyakit') ?>';
  ...
  $(document).on('click','.btn-edit', function(){
    ...
    Loader && Loader.show();
    $.get('<?= site_url('admin/master/kel-penyakit/get') ?>/'+id)
      .done(function(res){
        if(res.status==='ok'){
          $('#id').val(res.data.id);
  ...
  $('#formKelP').on('submit', function(e){
    ...
    Loader && Loader.show();
    $.ajax({
      url: '<?= site_url('admin/master/kel-penyakit/save') ?>',
