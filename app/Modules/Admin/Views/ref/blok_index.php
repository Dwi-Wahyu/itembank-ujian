...
<div class="d-flex align-items-center justify-content-between mb-3">
  <h2 class="page-title mb-0">Master Blok</h2>
  <button class="btn btn-primary" id="btnAdd">
    <i class="bi bi-plus-circle me-1"></i> Tambah Blok
  </button>
</div>
...
<div class="card mb-3">
  <div class="card-body">
    <form class="row g-2" id="frmSearch">
      <div class="col-md-6">
        <input type="text" class="form-control" name="q" value="<?= esc($q ?? '') ?>" placeholder="Cari nama blok…">
      </div>
      <div class="col-md-6 text-md-end">
        <button class="btn btn-outline-primary"><i class="bi bi-search"></i> Cari</button>
        <a href="<?= site_url('admin/master/blok') ?>" class="btn btn-link">Reset</a>
      </div>
    </form>
  </div>
</div>
...
<div id="listWrap">
  <?= view('\Modules\Admin\Views\ref\partials\blok_table', get_defined_vars()) ?>
</div>

<!-- MODAL: Tambah/Edit -->
<div class="modal fade" id="modalBlok" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog">
    <form class="modal-content" id="formBlok" autocomplete="off">
      <?= csrf_field() ?>
...
(function(){
  const $wrap  = $('#listWrap');
  const modalEl= document.getElementById('modalBlok');
  const modal  = new bootstrap.Modal(modalEl);

  function buildURL(){
    const base = '<?= site_url('admin/master/blok') ?>';
  ...
  $(document).on('click','.btn-edit', function(){
    ...
    Loader && Loader.show();
    $.get('<?= site_url('admin/master/blok/get') ?>/'+id)
      .done(function(res){
        if(res.status==='ok'){
          $('#id').val(res.data.id);
  ...
  $('#formBlok').on('submit', function(e){
    ...
    Loader && Loader.show();
    $.ajax({
      url: '<?= site_url('admin/master/blok/save') ?>',
